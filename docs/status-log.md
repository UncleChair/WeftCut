...
| Project mutations | `Project` | — | **No generic row per successful mutation, by design.** The history panel is that feed, and a per-commit producer would double-log every agent mutation the `Mcp` row above already summarises. What does land as `Project`: refusals (the direct-commits row above), the load-time repairs (the two rows below, plus schema upgrade), checkpoint create/delete and restore pin-rows (`source = User` or `Agent { client }`, whichever path called), a dropped transition reconcile, and motif staleness. A successful commit otherwise reaches the log only as a `Shortcut` row, when a command surface dispatched it. |
