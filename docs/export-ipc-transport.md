...
1. The export Worker composites into a render target sized for the target
   ...
   bytes. The readback is asynchronous (PBO + fence, `PboFrameReader`): the
   Worker submits frame N's readback, then retrieves frame N−1, whose fence
   has had a full frame to signal — so the CPU never stalls on GPU completion.
2. The Worker posts the retrieved frame to the renderer main thread over the export
   `chunk` channel (zero-copy `postMessage` transfer of the frame-owned buffer). The
   `chunk-ack` is the backpressure, with exactly one frame in flight: the Worker
   composites and packs ahead while the previous frame's ack is pending, and awaits
   that ack before posting — the transport round-trip overlaps GPU work instead of
   serializing after it, and the ack loop still bounds the pipeline at one unwritten
   frame.
