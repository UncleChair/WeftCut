...
the matrix; run it with `npm run fixtures` (from `apps/desktop/e2e`) to bring the
media up to date. Both scripts use only Node built-ins and invoke `ffmpeg`
without a command shell, so output directories with spaces and native Windows
paths are preserved. Requires `ffmpeg` on PATH.

Existence is not what decides whether a clip is current. Each entry's recipe
identity is the hash of the ffmpeg argv it would run, recorded with the entry's
complete output set in `media/manifest.json`; a pass regenerates exactly the
entries whose hash moved or whose files are incomplete, and a missing manifest
regenerates everything. There is deliberately no way to declare the media
trustworthy instead — editing a recipe and keeping the old clip is the failure
this replaced. Files no entry claims are reported and left alone.

Playwright's `globalSetup` runs the same pass before the suite boots, so a warm
checkout costs a sub-second no-op and nothing has to be generated by hand;
electron-ci additionally runs it as its own cached step, per OS, because the
drawtext font and the bundled ffmpeg build differ per platform.
