...
predicate naturally. The sweep also covers **project open**: a project authored
on a capable machine and opened on an incapable one re-probes its DirectExport
sources and corrects them.
...
3. Compute the **wait set** = referenced video sources whose
   `exportPlaybackPathFor` is `null` (proxy in-flight). If empty, launch the
   export Worker as today.

When the wait set is non-empty, `App` enters a new export state
`{ kind: "preparing", sources: [{ id, label }] }` (rendered by `ExportPanel`
next to the existing `starting | progress | complete | error`). It reuses the
existing `media:job_*` listener (already feeding `proxyState`) to show per-source
proxy progress, and when **all** wait-set proxies reach `proxy_path` set
(`proxyState` "ready"), it **automatically launches the export** — the user does
nothing. A `media:job_error` on a needed proxy transitions to
`{ kind: "error" }` with a real "failed to prepare <label>" message.
...
- **Export pre-flight + auto-wait:** pre-flight failure no longer throws; it
  route-corrects and produces a non-empty wait set. Wait set resolves to launch
  on all-proxies-ready; a needed proxy `job_error` produces an error state.
  Reference-range scoping: an unused undecodable import is **not** in the wait set.
