...
Status: **probes RUN 2026-06-12; the 10-bit export pipeline they settled
shipped to `main`.** The decision and its rationale are recorded in
**ADR 0022** (10-bit export: float16 composite + native encode exit);
the evergreen description lives in `docs/render.md` ("Encode exits").
This document is kept as the **probe record** — its HDR (P5) and
`importExternalTexture` (P6) findings seed the deferred v2 HDR work. The
loopback-WebSocket sink + `TenBitIngest`/`PackYuv420p10`/`TenBitFrame`
pipeline described in the decision tree below are implemented and
gate-passing. Probe vehicle:
`apps/desktop/e2e/tools/float16_probes.e2e.js` (re-runnable; the H.264
Hi10P fixture is generated by `fixtures/generate.go --gradient-h264`
as `media/test_1080p_gradient10_h264.mp4`).
