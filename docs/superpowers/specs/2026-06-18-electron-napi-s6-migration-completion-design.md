...
| **PerfHUD secondary window** | `src/electron-compat/tauri-webview-window.ts` is still a no-op stub; the PerfHUD popup never opens. | No (dev/diagnostic tool) | **Implement** a real second `BrowserWindow` behind the `tauri-webview-window.ts` shim (decided: implement, not degrade). Wire `show/hide/close/center` + `getByLabel` to a main-process secondary-window manager. |
