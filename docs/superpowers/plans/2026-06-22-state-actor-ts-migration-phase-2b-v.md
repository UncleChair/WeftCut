...
- Modify: `src/main/state/model.ts` (add the missing `MediaItem.label` field — see Step 1)
- Modify: `src/main/state/history.test.ts` (re-add `label: null` to its inline media fixture — see Step 1)
...
- [ ] **Step 1: Model-fidelity prerequisites (model.ts + history.test.ts + add.ts).** (Discovered during Task 1: the TS `MediaItem` is missing `label`, which the Rust wire shape always emits.)
  - `model.ts`: add `label: string | null` to the `MediaItem` interface (place it right after `id`). The Rust `MediaItem` has `pub label: Option<String>` (media.rs:15) with NO serde-skip, so it ALWAYS serializes `"label": null`; without this field the media corpus diverges from the oracle. It is the ONLY missing field (all 17 others already match).
  - `history.test.ts`: Task 1's implementer removed `label: null` from that file's inline `mediaItem(id)` fixture because the type rejected it. Now that the model has `label`, re-add `label: null` to that fixture (right after `id`) so it typechecks against the now-present field.
  - `add.ts`: change `function defaultTransform()` to `export function defaultTransform()` (add.ts:21) — `media.ts` imports it.
  - Verify `npm run typecheck` (from `apps/desktop/`) is clean after these three edits before continuing.
...
```bash
# npm run typecheck (clean)
git add apps/desktop/src/main/state/mutations/media.ts apps/desktop/src/main/state/mutations/media.test.ts apps/desktop/src/main/state/mutations/add.ts apps/desktop/src/main/state/model.ts apps/desktop/src/main/state/history.test.ts
git commit -m "feat(state-migration): media param-builders + separate_audio + media item template + MediaItem.label (Phase 2b-v)"
```
