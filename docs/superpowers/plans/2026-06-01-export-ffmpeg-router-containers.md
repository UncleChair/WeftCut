...
- [ ] **Step 4: Containers** — Export to **MOV** and **MKV**. Confirm output extension matches, file plays, `ffprobe` shows the right container + codec + AAC audio. Note: **AV1+MOV is invalid** (ffmpeg's MOV muxer rejects AV1 — `isCodecContainerValid` removes MOV from the container dropdown when AV1 is selected); verify AV1-in-**MKV** works (`ffprobe out.mkv` → `av1`) and that picking AV1 hides/replaces the MOV option.
