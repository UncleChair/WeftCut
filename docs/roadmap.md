...
| **Timeline UI** | ✅ MVP done (`apps/desktop/src/timeline/Timeline.tsx`). Time ruler with second tick marks, one row per track, layer blocks with stable per-layer color, click-to-select with white outline, drag layer body to move (calls `move_layer`), drag left/right edges to trim (calls `update_layer`), Delete key removes selected layer. Fixed zoom at 80 px/s. Track lanes coloured by kind. **Drag from media pool**: media-pool items are HTML5 draggable (`application/x-weftcut-media` payload), tracks accept the matching kinds (video tracks accept Video + Image, audio tracks accept Audio), drop spawns a layer at the cursor's `t_start_us` via the new `add_media_layer` Tauri command. **Pending**: scrub indicator + click-to-seek, zoom control, multi-select, cross-track drag of existing layers. |
| **libmpv preview connected to live filter graph** | ✅ done (Windows; mac/Linux still standalone). Install path automated: `apps/desktop/src-tauri/vendor/libmpv/` is populated by the script that downloads Shinchiro's libmpv-dev archive, generates `mpv.lib` from `libmpv-2.dll` exports via `dumpbin` + `lib.exe`, and drops the headers in place. `build.rs` extends `rustc-link-search` and stages `libmpv-2.dll` into `target/<profile>/`. `libmpv2 = { optional = true }` behind the `mpv` Cargo feature; `npm run dev`'s default already passes `--features mpv`. **Project preview embeds** into a child HWND we create at startup as a sibling of WebView2 (commits `4d9fcc0` + `b5fdc7a`, 2026-05-12) — `ensure_init` sets `wid` before the first `loadfile`; React streams `getBoundingClientRect() * devicePixelRatio` via `mpv_set_surface_rect` → `SetWindowPos(HWND_TOP, …)`. The preview pane has a centered transport row at the bottom (⏮ / ⏯·⏸ / ⏭). **Media-pool / raw-file preview** keeps the popup top-level-window UX via a second `MpvPopupSlot` (no `host_hwnd` registered → `ensure_init` falls back to `force-window=yes`). Hot-reload of `lavfi-complex` on every project commit landed in Phase 1. **Pending**: libmpv → UI playhead-position feedback so the ruler tracks playback in real time; mac/Linux embed (NSView / GtkBox) — both slots stay on the standalone fallback on those platforms. |
| **Export pipeline** (originally Phase 3) | ✅ MVP done in `src/export/`. Drives ffmpeg via `tokio::process::Command`, writes the IR's filter graph to `%TEMP%/weftcut-export-<id>.txt`, parses `-progress pipe:1` for per-block updates, and re-emits them as Tauri events (`export:progress` / `export:complete` / `export:error`). UI: **Export…** action button + a bottom-left progress panel with percent/frame/fps/speed, full i18n. Preset is hard-wired to `libx264 -preset medium -crf 20` + AAC 192k MP4. Hardware-encoder probing, render queue, ProRes/GIF presets remain Phase 3. **Needs ffmpeg installed.** |
...
| 7 — Import-time fan-out | ✅ Both `commands::import_media` and `WeftCutServer::import_media` call `jobs::enqueue_for_media` after `add_media_item` succeeds. Identical event streams from either source. |
...
Forward motion: as of 2026-05-12, Phases 0–4 + 6 are closed. **Phase 2 deferrals closed 2026-05-11** (composition order, crossfade transitions; mpv drawtext preview regression-locked but runtime-unverified — likely already fixed by side effect, needs interactive dev-session confirmation). **Phase 4.x cleanup partial 2026-05-11** — MCP activity panel + `detect_silences` tool + `/cut-silences` prompt shipped; `dry_run` MCP tool deferred (needs `do_*` method refactor to split pure mutation from commit, or parallel `apply_*` functions for the supported ops — not a tail-of-session move). **Phase 6 closed 2026-05-11.** One scope gap from Phase 6 still deferred: omitting unconfigured cloud tools from `list_tools` (rmcp 0.1.x `tool_box` macro limitation). Other open slices: Phase 5 (rasterized templates), Phase 7 polish (timeline waveform display, media-pool thumbnail strip), Phase 4.x `dry_run` if/when the agent UX hits the "I want to test a plan before committing" wall. 175 lib tests passing.
...
| Text / DrawText | ✅ IR + emitters + property-panel content/font/size/color/x-y/opacity controls. Export path works end-to-end (post `;`-separator fix — see "emit_ffmpeg fix" below). Live preview through mpv verified at runtime 2026-05-12 (graph loads clean, text renders on video) — the `Raw(-13)` regression was already fixed by side effect of earlier libmpv2 arg-quoting + path-escape work; we just hadn't re-run interactively. |
...
### Phase 2 deferrals — all closed by 2026-05-12

- **Crossfade / dissolve between two clips** — ✅ shipped 2026-05-11 (commit `cd2f6e5`). New `state/transition.rs` with `Transition { id, from_layer, to_layer, duration_us, kind }` and `Project::transitions: imbl::Vector<Transition>` (back-compat via `#[serde(default)]`). The `LayerOverlap` validator exempts overlapping layers iff a transition references both and `duration_us` equals the overlap. `add_transition` is atomic: takes back-to-back layers and extends `from_layer.t_end_us` (and `src_out_us` for media-bearing kinds) in the same commit. IR gets `IRNode::Fade.alpha: bool` (passes `:alpha=1` to ffmpeg's fade filter) and new `IRNode::Format { in_, pix_fmt }` for the `format=yuva420p` pre-fade conversion; lowering threads an `incoming_transition_map` so the incoming layer gets an alpha fade-in over its first `duration_us`. End-to-end smoke `crossfade_transition_renders_through_ffmpeg` (+14 tests total).
- **mpv drawtext + image-overlay live preview** — ✅ verified working 2026-05-12. Interactive runtime smoke through the dev shell with `cargo run --features mpv`: text-on-video graph (`color [c1] → drawtext [dt] → [s] format=yuv420p [vo]` shape, `fontfile='C\:/Windows/Fonts/arial.ttf'` double-escaped) loads with no `Raw(-13)`, no `Fontconfig error:`, and the text actually renders on the video in the preview window. The regression was already fixed by earlier sessions — most likely the libmpv2 0.4.1 `command()` arg-quoting fix + the `\` → `/` path normalization for primary inputs. The emit-shape regression test `text_on_video_emits_drawtext_with_double_escaped_fontfile` locks the failing-graph properties in. Image overlay live preview presumed fixed by the same side effect (same failure mode; not re-tested independently).
- **Layer composition order** — ✅ closed 2026-05-12 (commit `a5d3ef3`). The lowering convention "tracks[0] = bottom of z-stack" is correct and matches Premiere/Resolve/FCP; the user-visible problem was that the default tracks were labelled in reverse — `Project::new_blank` shipped `imbl::vector![b_roll, a_roll]`, putting "B roll" at the bottom of z-stack and "A roll" on top, which inverts the NLE convention that A-roll is the base video. Fixed by swapping to `imbl::vector![a_roll, b_roll]` so A-roll lands at index 0 (bottom = base video) and B-roll at index 1 (top = overlays). No lowering/IR changes; only the default label-to-position mapping for new projects. `blank_project_ships_with_a_b_roll` tightened to assert positions, not just label presence.
...
| Preview thumbnails | `ea9744d` (2026-05-13) | Closes the documented picker-thumbnails deferral. One render path, two surfaces: MCP resource `templates://<id>/preview` (returns PNG as BlobResourceContents for agents) and Tauri command `template_preview(template_id)` (base64 for the picker's card grid via new `TemplateCardThumbnail` component). Both share `representative_preview_time_s = (default_duration_s × 0.5).clamp(0, 1)` — settled past the typical 0.3–0.6s entrance, never overshoots into exits. fps=1 with a single-time capture keeps the cache key from colliding with future emit-time renders. Picker card grid swaps from iframe (loading fallback) to static `<img>` once each render lands; form's large preview keeps the iframe for live prop-edit animation. 196 → 200 lib tests (+4 preview-time heuristic). |
...
| **Preview thumbnails for templates** (`templates://{id}/preview`) | ✅ shipped 2026-05-13 (`ea9744d`). On-demand render through the same raster pipeline IR Template layers use; result cached via `raster::cache_key` so first-call cost (~200–700ms cold) is paid once per template per session. Picker uses the Tauri-command twin (`template_preview`) for the card grid, falling back to iframe while each render is in flight. |
...
1. ✅ **Keyring + Settings UI** (commit `9b84059`). `cloud/keys.rs` with
   ...
   `"weftcut"`, username = lowercase provider tag. Panel iterates
...
3. ✅ **`apply_subtitles` MCP tool** (commit `05fa8df`). Body-based
   ...
   track via new `WeftCutServer::ensure_subtitle_track` (mirrors the
