...
export function DropStrip({
  ...
  const armed = activeMediaDrag !== null;
  const lit = visibleDropPreview !== null;
  const ghostLeftPx = lit
    ...
    >
      {visibleDropPreview !== null && (
        <>
          ...
          />
          {/* Anchored to the ghost's head rather than to the row, so the hint
              lands beside the pointer at any scroll offset, and offset far
              enough to clear the absorbed media chip (which reaches at most
              MEDIA_DRAG_CURSOR_OFFSET_PX + 18 px past the head). Absolute
              because a 14 px row's height must never depend on the text. */}
          <div
            ...
            style={{ left: ghostLeftPx + MEDIA_DRAG_CURSOR_OFFSET_PX + 24 }}
          >
