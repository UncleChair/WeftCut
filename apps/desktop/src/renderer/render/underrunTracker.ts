// Playback underrun detection — the "dropped frames" signal behind the
// transport-bar indicator (and, later, res-throttle / stop-on-drop).
//
...
// the only trace of an underrun is a console log. `judgeFrameSelection`
// classifies each painted VideoClip frame as fresh or late;
// `UnderrunTracker` aggregates per-sweep verdicts into a per-play-session
// dropped-frame count and an edge-triggered "active" flag safe to feed
// into React state.
...
export interface UnderrunSnapshot {
  /// True while an underrun was observed within the last `holdMs` —
  /// drives the indicator's "lit" state. Held on briefly after recovery
  /// so a flickering ring doesn't strobe the UI.
  active: boolean;
  /// Comp frames painted late since the current play session started.
  /// Persists after pause (Premiere-style: the count stays visible until
  /// the next play) and resets on the next `beginPlay()`.
...
export interface UnderrunTrackerInit {
  ...
  /// How long `active` stays true after the last late frame.
  holdMs?: number;
...
export class UnderrunTracker {
  ...
  /// In-play seek grace: true while late sweeps are suppressed. Cleared
  /// by the first all-fresh sweep (pipeline re-primed) or by
  /// `graceDeadlineMs` (so a genuinely wedged decoder can't hide behind
  /// an eternal grace).
  ...
  /// so the following ~ring-rebuild interval would otherwise count as
  ...
  /// `anyLate` = at least one visible VideoClip painted late; `frameUs`
  /// is the snapped comp time that was judged.
  judgeSweep(anyLate: boolean, frameUs: number): void {
    ...
    if (this.graceArmed) {
      if (!anyLate) {
        ...
        this.graceArmed = false; // grace expired with the decoder still behind
      }
    }
    if (!anyLate) return;
    if (frameUs !== this.lastDropFrameUs) {
  ...
  /// indicator dims after the last late frame even once judging stops.
  tickDecay(): void {
    if (this.active && this.now() >= this.activeUntilMs) {
      this.active = false;
      this.emit(true);
    }
  }
  ...
  snapshot(): UnderrunSnapshot {
    return { active: this.active, droppedFrames: this.droppedFrames };
  }

  /// Session-end summary for the LogBus row: the dropped count, at most
  /// once per play session (0 on repeat calls until the next
  /// `beginPlay()`).
  takeSessionSummary(): number {
    if (this.sessionConsumed) return 0;
