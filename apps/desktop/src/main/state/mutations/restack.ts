 ...
 *  order, so the op moves lanes, not indices within one: `position` puts the
 *  mover's lane directly above/below the track the ANCHOR layer sits on,
 ...
 *  neighbour, a co-resident audio layer) splits onto a fresh lane at the
 ...
 *
 *  Returns the id of the lane the mover ends up on (the moved track, or the
 *  freshly minted one), or null when the mover's lane is ALREADY directly at
 ...
 *  lane's id is minted only after every check, so a refusal burns no id. */
export function applyRestackLayer(p: Project, idGen: IdGen, layerId: Uuid, anchorId: Uuid, position: RestackPosition): Uuid | null {
  ...
  const [mi] = moverLoc
  const [ai] = anchorLoc
  // Already directly at the requested side of the anchor's lane — in z terms
  // the layer already sits where the caller asked, whichever branch would have
  // carried it there, so nothing happens (and a shared lane is not split
  ...
  if (src.layers.length === 1 && src.role === null) {
    // Sole occupant on an ordinary lane: the lane IS the layer's z, so the
    // lane moves whole and its identity — id, label, lock, height — survives.
  ...
  // Split path: the mover leaves alone. `label: null` lets the fresh lane
  ...
  // Cleanup rides in the SAME mutation (one undo restores layer + lane +
  // pruned source together). The predicate declines the reserved skeleton and
  // locked lanes on its own — one rule, one home.
