 ...
 *  result (a media id for import_media). Throws on a rejected actor write or an
 *  unhandled tool. */
export async function runHybrid(tool: string, args: Record<string, unknown>, deps: HybridDeps): Promise<unknown> {
  switch (tool) {
    case 'import_media': {
      const path = args.path as string
      // Subtitles are CONSUMED into a caption track, not pooled. The subtitle
      // hybrid + this branch's body land in Task 4; until then, branch (so we
      // never probe a .srt as media) and signal the deferral explicitly.
      if (/\.(srt|ass|vtt)$/i.test(path)) throw new Error('runHybrid: subtitle import is not yet wired (Task 4)')
