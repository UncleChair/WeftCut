...
if (plat === 'win32') {
  ...
  const version = '7.1.1'
  const zipName = `ffmpeg-${version}-essentials_build.zip`
  const url = `https://github.com/GyanD/codexffmpeg/releases/download/${version}/${zipName}`
  const zipPath = join(tmp, zipName)

  console.log(`Downloading ffmpeg ${version} (Windows essentials) from gyan.dev/GitHub...`)
  ...
  const extractDir = join(tmp, `ffmpeg-${version}-essentials_build`)
  execSync(`tar -xf "${zipPath}" -C "${tmp}" "${`ffmpeg-${version}-essentials_build/bin/ffmpeg.exe`}"`, { stdio: 'inherit' })
