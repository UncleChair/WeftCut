...
describe("motif renders + animates in export (real WebView2)", function () {
  ...
  it("exports a countdown motif whose frames differ across seconds", async () => {
    ...
    // 3) Add a 2s countdown motif (60 frames @ 30fps) and export it, FIRE-
    ...
    await browser.execute((output) => {
      ...
      window.__weftcutTest
        ...
        .then(() => {
          window.__e2eMotifDone = { ok: true };
        })
        .catch((e) => {
          window.__e2eMotifDone = { ok: false, error: String(e) };
        });
    }, OUTPUT);

    // 4) Poll until settled. The phase passes through preparing (the motif
    ...
    try {
      await browser.waitUntil(
        async () => {
          ...
          if (snap.frame != null && snap.frame !== lastFrame) {
            ...
            console.log(
              `[e2e] motif export ${snap.kind}/${snap.phase ?? "-"} frame=${snap.frame}`,
            );
          }
          if (snap.kind && snap.kind !== lastKind) {
            lastKind = snap.kind;
            console.log(`[e2e] motif export phase -> ${snap.kind}`);
          }
          if (snap.done) {
            settled = snap.done;
            return true;
      ...
      throw new Error(
        `motif export never settled (last kind=${lastKind}, last frame=${lastFrame}): ${e.message}`,
      );
    }
